...
resource "aws_ssm_association" "cwagent_start_windows_ssm_param" {
  ...
  parameters = {
    commands = [
      "& 'C:\\Program Files\\Amazon\\AmazonCloudWatchAgent\\amazon-cloudwatch-agent-ctl.ps1' -a fetch-config -m ec2 -c ssm:/GoAnywhere-pr/ec2/amazon-cloudwatch-agent/config/windows -s"
    ]
  }
